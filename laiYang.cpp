...
void msgComplex(){
    ...
    sprintf(log, "%s\nMessage complexity for Lai-Yang Algo : %d.\n", log, sum);
}
        ...
        if(FD_ISSET(serverSocket, &readfds))
        {
            ...
            for(i=0;i<maxClients;i++){
                if(clientList[i] == 0){
                    ...
                    if(rmsgtype==0 && color == 'w' && rmsg.col =='w'){
                        mtx.lock();
                        bal += rmsg.amt;
                        gettime();
                        me++;
