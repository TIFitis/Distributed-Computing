...
int searchQueue(int key){
    for(int i = 0; i < token.queue_front; i++)
        if(token.req_queue[i] == key)return 1;
    return 0;
}
...
void enqueue(int key){
    for(int i=token.queue_front;i>=0;i--)token.req_queue[i+1] = token.req_queue[i];
    token.req_queue[0] = key;
    token.queue_front++;
}

//Remove a process id from the queue.
int dequeue(){
    token.queue_front--;
    return token.req_queue[token.queue_front];
}
...
void sendMessage(){
    ...
    if(smsgtype == 0 || smsgtype == 2){
        ...
        gettime();
        sprintf(log, "%s\np%d sends token to p%d at %s", log, mypid, token.did, tim);
        struct sockaddr_in serv_addr;
        sock = socket(AF_INET, SOCK_STREAM, 0);
        memset(&serv_addr, '0', sizeof(serv_addr));
        serv_addr.sin_family = AF_INET;
            ...
            for(i=0;i<maxClients;i++){
                if(clientList[i] == 0){
                    ...
                    if(rmsgtype == 0){
                        ...
                        if(rreq.req_no > req[rreq.sid])req[rreq.sid] = rreq.req_no;
                        cout << "Recieved request from " << rreq.sid << endl;
                        gettime;
                        sprintf(log, "%s\np%d recieves p%d's request to enter CS at %s", log, mypid, rreq.sid, tim);
                        mtx4.lock();
                        if(!executing_cs && have_token){
                            if(req[rreq.sid] == token.last[rreq.sid] + 1){
                                cout << "Not in cs so sending token to " << rreq.sid << endl;
                                have_token = 0;
                                token.sid = mypid;
                                token.did = rreq.sid;
                                smsgtype = 1;
                        ...
                        gettime();
                        sprintf(log, "%s\np%d recieves token from p%d at %s", log, mypid, token.sid, tim);
...
void *getPid(void *tid){
    ...
    if(mypid == initial){
        have_token = 1;
        cout << "init = " << initial << " mypid = " << mypid << endl;
    }
...
void executeCS(){
    ...
    token.last[mypid] = req[mypid];
    cout << "left CS" << endl;
    for(int i=0;i<nt;i++){
        if(req[i] == token.last[i]+1 && !searchQueue(i)){
            enqueue(i);
        }
    }
    if(token.queue_front){
        mtx.lock();
        token.sid = mypid;
        token.did = dequeue();
        smsgtype = 1;
...
void autoWork(){
    ...
    for(int i=0;i<k;i++){
        ...
        if(!have_token){
            sprintf(log, "%s\np%d requests to enter CS at %s for %d time", log, mypid,  tim, req[mypid]);
            for(int i = 0;i<nt;i++){
                if(i!=mypid){
                    sreq.did = i;
                    sreq.sid = mypid;
                    smsgtype = 0;
                    sreq.req_no = req[mypid];
                    mtx.lock();
                    sendMessage();
                    mtx.unlock();
                }
            ...
            sprintf(log, "%s\np%d already has token at %s for %d time", log, mypid,  tim, req[mypid]);
        while(!have_token);
        executing_cs = 1;
        executeCS();
        executing_cs = 0;
