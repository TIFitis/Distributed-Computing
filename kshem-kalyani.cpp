        ...
        if(FD_ISSET(serverSocket, &readfds))
        {
            ...
            for(i=0;i<maxClients;i++){
                if(clientList[i] == 0){
                    ...
                    lu[mypid] = vt[mypid];
                    cout << "Recieved" << endl;
                    for(int i=0;i<rmsg.ntuple;i++){
                        cout << "{" << rmsg.kkdata[i].pid << " , " << rmsg.kkdata[i].clock << "}";
                        if(vt[rmsg.kkdata[i].pid]<rmsg.kkdata[i].clock && i!= mypid)vt[rmsg.kkdata[i].pid] = rmsg.kkdata[i].clock, lu[rmsg.kkdata[i].pid] = vt[mypid];
                    }
                    cout << endl;
                    gettime();
                    me++;
...
void sendMessage(){
    ...
    me++;
    ms++;
    md+=smsg.ntuple*sizeof(int);
    struct sockaddr_in address;
    int sock = 0;
    cout << "Sent message to " << smsg.did << endl;
...
void autoWork(){
    ...
    while(m>0){
        ...
        while(b2-- && m--){
            ...
            for(int i=0;i<nt;i++){
                if(ls[smsg.did]<lu[i]){
                    smsg.kkdata[smsg.ntuple].pid = i;
                    smsg.kkdata[smsg.ntuple].clock = vt[i];
                    cout << "{" << smsg.kkdata[smsg.ntuple].pid << " , " << smsg.kkdata[smsg.ntuple].clock << "}";
                    smsg.ntuple++;
                }
            }
            cout << endl;
